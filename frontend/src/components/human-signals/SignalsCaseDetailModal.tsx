    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-border bg-surface shadow-2xl">
        ...
        <div className="p-6">
          ...
          {messages.length > 0 && (
            <div className="mt-4">
              ...
              >
                <div className="max-h-96 space-y-3 overflow-y-auto rounded-lg border border-border bg-gray-50 p-4 dark:bg-gray-900">
                  {messages.map((msg, i) => {
        ...
        <div className="border-t border-border px-6 py-4">
          <button
            onClick={onClose}
            className="w-full rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-text-primary transition-colors hover:bg-gray-200 dark:bg-gray-800"
          >
            Close
          </button>
        </div>
...
function ExpandableSection({
  ...
  return (
    <div>
      <button
        onClick={onToggle}
        className="flex w-full items-center justify-between rounded-lg border border-border bg-gray-50 px-4 py-3 text-left transition-colors hover:bg-gray-100 dark:bg-gray-800 dark:bg-gray-900 dark:hover:bg-gray-700"
      >
...
function StructuredEntry({ label, value }: { label: string; value: unknown }) {
  ...
  if (value != null && typeof value === 'object' && !Array.isArray(value)) {
    ...
    return (
      <div className="rounded-md border border-border bg-gray-50 p-3 dark:bg-gray-900/50">
        <div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-text-muted">
          {label}
        </div>
    ...
    return (
      <div className="mt-1 flex flex-wrap gap-1.5">
        {cleaned.map((item, i) => (
          <span
            key={i}
            className="inline-flex rounded-md border border-border bg-gray-50 px-2 py-0.5 text-xs text-text-primary dark:bg-gray-900"
          >
            {item}
          </span>
        ))}
    ...
    <div className="mt-1 space-y-1.5">
      {cleaned.map((item, i) => (
        <div key={i} className="flex items-start gap-2">
          <span className="mt-0.5 flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 text-[10px] font-medium text-text-muted dark:bg-gray-800">
            {i + 1}
          </span>
