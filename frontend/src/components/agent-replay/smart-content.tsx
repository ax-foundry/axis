...
export function SmartContent({ text, forceType }: { text: string; forceType?: 'json' }) {
  ...
  if (segments.length === 1) {
    ...
    if (seg.type === 'code') {
      return (
        <pre className="overflow-x-auto whitespace-pre-wrap rounded bg-gray-100 dark:bg-gray-800 p-2 font-mono text-[11px] leading-relaxed">
          <code>{seg.value}</code>
        </pre>
      );
    }
    ...
    <div className="space-y-2">
      {segments.map((seg, i) =>
        seg.type === 'json' ? (
          ...
          <pre
            key={i}
            className="overflow-x-auto whitespace-pre-wrap rounded bg-gray-100 dark:bg-gray-800 p-2 font-mono text-[11px] leading-relaxed"
          >
            <code>{seg.value}</code>
          </pre>
        ) : (
