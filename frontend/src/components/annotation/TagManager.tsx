          ...
          <button
            ...
            className={cn(
              ...
              isSelected
                ...
                : 'border-border bg-surface text-text-secondary hover:border-text-muted hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900'
            )}
          >
            {tag}
          </button>
    ...
    <div className={cn('relative', className)}>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-lg border border-gray-200 dark:border-gray-700 bg-surface p-4 shadow-lg">
              ...
              <div className="flex flex-wrap gap-2">
                {(Object.keys(TAG_PRESETS) as TagPreset[]).map((preset) => (
                  <button
                    ...
                    className="rounded-md bg-gray-100 dark:bg-gray-800 px-2.5 py-1 text-xs capitalize text-text-secondary hover:bg-gray-200"
                  >
                    {preset}
                  </button>
                ))}
            ...
            <div className="mb-4">
              ...
              <div className="flex max-h-32 flex-wrap gap-1.5 overflow-y-auto">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1 rounded bg-gray-100 dark:bg-gray-800 px-2 py-1 text-xs text-text-secondary"
                  >
                    {tag}
